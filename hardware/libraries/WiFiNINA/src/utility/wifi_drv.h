...
class WiFiDrv
{
  ...
  public:
     ...
     * is determined by the size of the key (5 bytes for WEP-40, 13 bytes for
     * WEP-104).
     *
     * param ssid: Pointer to the SSID string.
     * param ssid_len: Length of SSID string.
     ...
     * 					 i.e. validParams = 1 means that we'll
     * change only IP address validParams = 3 means that we'll change IP address,
     * gateway and netmask param local_ip: 	Static IP configuration param
     * gateway: 	Static gateway configuration param subnet: 	Static subnet
     * mask configuration
     */
     ...
     * param validParams: set the number of parameters that we want to change
     * 					 i.e. validParams = 1 means that we'll
     * change only dns_server1 validParams = 2 means that we'll change dns_server1
     * and dns_server2 param dns_server1: Static DNS server1 configuration param
     * dns_server2: Static DNS server2 configuration
     */
     ...
     *
     * param networkItem: specify from which network item want to get the
     * information
     *
     ...
     * information
     *
     * return: signed value of RSSI of the specified item on the networks scanned
     * list
     */
     ...
     * information
     *
     * return: encryption type (enum wl_enc_type) of the specified item on the
     * networks scanned list
     */
    ...
    static int8_t wifiSetApPassphrase(const char *ssid, uint8_t ssid_len, const char *passphrase, const uint8_t len,
                                      uint8_t channel);
    static int8_t wifiSetEnterprise(uint8_t eapType, const char *ssid, uint8_t ssid_len, const char *username,
                                    const uint8_t username_len, const char *password, const uint8_t password_len,
                                    const char *identity, const uint8_t identity_len, const char *ca_cert,
